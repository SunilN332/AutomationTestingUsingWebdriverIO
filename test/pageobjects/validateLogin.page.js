...
class loginPageValidation extends Page {
    ...
    get accecptedUserNames() {
        return $('//div[@id="login_credentials"]/h4');
    }
    get passwordForAllUsers() {
    ...
    async validateLoginPage() {
        ...
        await expect(this.accecptedUserNames).toHaveText(
            expect.stringContaining('Accepted usernames are:')
        );
        await expect(this.passwordForAllUsers).toHaveText(
            expect.stringContaining('Password for all users:')
        );
